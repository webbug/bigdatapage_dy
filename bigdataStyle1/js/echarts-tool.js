...
function wbyb(){
    ...
    var myMap=echarts.init(document.querySelector('#webyb'))
...
//生成进度条
function BarInit() {
          var BarInit = echarts.init(document.getElementById('echartsBar'));// 初始化echarts实例
          ...
          var option={
              ...
              yAxis: {
                ...
                data: ['完成率','待处理','已完成']//类目数据，在类目轴（type: 'category'）中有效。
              ...
              series: [//系列列表。每个系列通过 type 决定自己的图表类型
                {
                  ...
                  data: [80,40,60],//系列中数据内容数组
                  label: { //图形上的文本标签
                    show: true,
                    position: 'right',//标签的位置
                    offset: [0,-13],  //标签文字的偏移，此处表示向上偏移40
                    formatter: '{c}{a}',//标签内容格式器 {a}-系列名,{b}-数据名,{c}-数据值
                    color: function(params) {//标签字体颜色
				  			var colorList = ["#edcd44", "#1cadbd", "#1cadbd"];
          ...
          BarInit.setOption(option);
          window.addEventListener("resize", function() {
          	BarInit.resize()
          })
}
function BarInit2() {
          var BarInit = echarts.init(document.getElementById('echartsBar2'));// 初始化echarts实例
  ...
  // 问题反馈类型占比
            function questionFn() {
                var numArr = [
                    {name: '高发时段1', value: 300},
                    {name: '高发时段2', value: 500},
                    {name: '高发时段3', value: 400},
                    {name: '高发时段4', value: 350},
                    {name: '高发时段5', value: 363},
                    {name: '高发时段6', value: 800},
                ]
                ...
                var option = {
                    ...
                    color:['#1a9eac','#37D2D4', '#d8bb3e', '#a0c52b', '#c57127'],
                    legend: {
...
function wxyg() {
	...
	option = {
		...
		series: [{
				name: '月',
				type: 'bar',
				color: 'rgba(28,173,189,1)',
				data: [90, 80, 70, 68, 65, 64, 60, 57, 68, 60, 64, 10],
				barWidth: '25%',
				itemStyle: {
					normal:{
			...
			},
			{
				...
				type: 'bar',
				color: 'rgba(28,173,189,1)',
				data: [70, 68, 65, 64, 60, 57, 68, 65, 64, 60, 70, 70],
				barWidth: '25%',
				itemStyle: {
					normal:{
						barBorderRadius: [5,5,0,0]
			...
			},
			{
				...
				type: 'bar',
				color: 'rgba(28,173,189,1)',
				data: [300, 270, 180, 350, 140, 300, 70, 50, 40, 38, 30, 90],
				barWidth: '25%',
				itemStyle: {
					normal:{
						barBorderRadius: [5,5,0,0]
...
function wwbzl() {
	...
	var symbolSizeData = [60, 50, 120, 100, 54];
	var datas = [];
...
function bjqs() {
	...
	var myChart = echarts.init(document.querySelector("#bjqs"));
...
function gfsd() {
	// 基于准备好的dom，初始化echarts实例
	var myChart = echarts.init(document.querySelector("#gfsd"));
	// (1)准备数据
	var data = {
		year: [
...
function rcxc() {
	// 基于准备好的dom，初始化echarts实例
	var myChart = echarts.init(document.querySelector(".rcxc"));
...
function dwtj() {
	// 基于准备好的dom，初始化echarts实例
	var myChart1 = echarts.init(document.querySelector('.dttj_lie1'));
	// 指定图表的配置项和数据
	var option1 = {
		title: {
			text: '消防设施',
		...
		color: ['#1a9eac', '#d8bb3e', '#a0c52b', '#fff'],
		series: [{
			name: '消防设施',
			type: 'pie',
			radius: ['69%', '81%'], // 半径
	...
	// 基于准备好的dom，初始化echarts实例
	var myChart2 = echarts.init(document.querySelector('.dttj_lie2'));
	// 指定图表的配置项和数据
	var option2 = {
		title: {
			text: '消防设备',
